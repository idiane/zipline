...
class WithBcolzDailyBarReader(WithEquityDailyBarData, WithTmpDir):
    """
    ...
    EQUITY_DAILY_BAR_SOURCE_FROM_MINUTE : bool
        If this flag is set, `make_equity_daily_bar_data` will read data from
        the minute bar reader defined by a `WithBcolzMinuteBarReader`.
    ...
    # allows WithBcolzDailyBarReaderFromCSVs to call the
...
class WithBcolzMinuteBarReader(WithEquityMinuteBarData, WithTmpDir):
    """
...
class WithAdjustmentReader(WithBcolzDailyBarReader):
    ...
    make_adjustment_writer_equity_daily_bar_reader() -> pd.DataFrame
        ...
        ``WithBcolzDailyBarReader``. This should probably not be
